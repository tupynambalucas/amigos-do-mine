...
| Status: | Documento Vivo (Atualizado em Maio de 2026) |
| :--- | :--- |
...
*   **Configuração do Ciclo:** O administrador define e agenda janelas de compra (ciclos) através da interface administrativa do `@elo-instance/web`.
*   **Ingestão de Produtos:** Uma lista de produtos em texto puro é processada por um parser inteligente no frontend e enviada para a `@elo-instance/api`.
*   **Abertura de Compras:** O catálogo estruturado é habilitado automaticamente para os membros na loja `@elo-instance/web` na data agendada.
*   **Execução de Pedidos:** Os membros montam cestas personalizadas e enviam seus pedidos através da loja `@elo-instance/web`.
*   **Processamento de Pagamento:** Integração direta com a API do **Banco EFI** via `@elo-instance/api` para pagamentos via Pix.
...
## Fase 2: Diretrizes Operacionais e Padrões
...
## Fase 4: Identidade de Marca & Design System
...
## Fase 6: Escalonamento de Longo Prazo para SaaS (Pós-v1.0)
